...
func main() {
	...
	name = "理想"
	fmt.Println(name)
	var ages [30]int // 声明了一个变量ages,它是[30]int整型
	...
	m1["理想"] = 100
	fmt.Println(m1)
	fmt.Println(m1["jiwuming"]) // 如果key不存在,返回的是value对应类型的零值
	...
	if !ok {
		...
		fmt.Println("jiwuming的分数是", score)
	}

	delete(m1, "lixiang") // 删除的key不存在则什么都不干
	delete(m1, "理想")
	fmt.Println(m1)
